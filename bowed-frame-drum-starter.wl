...
(* ============================================================
   ...
   ============================================================
   f_{mn} = (alpha_{mn} / (2 Pi r)) * c_membrane
   where c_membrane = sqrt(T / sigma)
         T = surface tension [N/m]
         sigma = surface mass density [kg/m2]
         alpha_{mn} = n-th zero of Bessel J_m

   For a goatskin head at bodhran tension:
     sigma_head ≈ 0.35 kg/m2  (goatskin ~350 g/m2)
     Target f01 = 130 Hz → solve for T
*)

headRadius = 0.215;   (* m = 430 mm / 2 *)
sigma_head = 0.35;    (* kg/m2 goatskin surface mass density *)
...
targetF01 = 130.0;
T_head = sigma_head (2 Pi headRadius targetF01 / 2.4048)^2;

(* Membrane wave speed *)
c_membrane = Sqrt[T_head / sigma_head];

(* Compute all mode frequencies *)
membraneFreqs = Table[
  {StringJoin["f", ToString[row[[1]]], ToString[row[[2]]]],
   row[[1]], row[[2]], row[[3]],
   N[(row[[3]] / (2 Pi headRadius)) * c_membrane]},
  {row, besselZeros}
];
...
Print[membraneTable];
Print["Surface tension T = ", NumberForm[T_head, {5,2}], " N/m"];
Print["Membrane wave speed = ", NumberForm[c_membrane, {5,1}], " m/s"];
...
barL = 0.400;   (* m tone bar length *)
barW = 0.025;   (* m tone bar width (bending axis) *)
barH = 0.001;   (* m tone bar thickness *)

I_bar = barW * barH^3 / 12;           (* second moment of area m^4 *)
A_bar = barW * barH;                   (* cross-section area m^2 *)
...
barFreqs = Table[
  N[(lambda2[[n]] / (2 Pi barL^2)) * Sqrt[kappa_sq]],
  {n, 1, 4}
];
...
wheelOD = 0.050;    (* m *)
vRimMin = 0.10;     (* m/s slow bow *)
vRimMax = 0.80;     (* m/s fast bow *)
fRimMin = vRimMin / (Pi * wheelOD);
fRimMax = vRimMax / (Pi * wheelOD);

Print["\n=== Rosined Wheel Excitation ==="];
Print["Wheel OD = ", wheelOD*1000, " mm"];
...
Print["head_surface_tension_N_per_m,", NumberForm[T_head, {5,1}]];
